...
Scripts numbered, run in order. Start with dependencies:
...
Scripts single-responsibility: one script = one task. Rmd/notebook files orchestrate and call prior scripts for final output.
...
**Script numbering:** Files depending on others get numeric prefix (`00`–`10`). Functions and sourced scripts go at top of every file.

No hard-coded values. Repeated code → parameterized function.