...
combined$majority_eth = relevel(factor(combined$majority_eth), ref = "white")
combined$majority_race = relevel(factor(combined$majority_race), ref = "white")
...
# how much access to emergency food is there (+ is GOOD)
summary(lm(index ~ total_popE + perc_black + perc_hisp + 
             food_insecure_percentage + supply_gap_lbs_natural, 
           data = combined))

# does the area majority impact the food insecurity index?
