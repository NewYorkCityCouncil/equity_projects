...
# ---- correlations: fine particles (PM 2.5) x race/ethnicity ----
# # ---- white alone 
# w_plot <- ggplot(data = community_districts, 
#                  aes(x = pct_white_alone/100, y = annual_mean_mcg_m3, color = pct_white_alone,
...
# white_pm_mp <- girafe(ggobj = combined, width_svg = 12, height_svg = 5.5) %>%
...
#                  opts_toolbar(pngname = "white_pm_corr"))

# # ---- asian alone
# a_plot <- ggplot(data = community_districts, 
#                  aes(x = pct_asian_alone/100, y = annual_mean_mcg_m3, color = pct_asian_alone,
#                      data_id = communitydist)) +
#   stat_cor(method = "spearman") + 
#   geom_point_interactive(size = community_districts$total_pop/40000,
#                          aes(tooltip = paste0("<strong>Community District: ", geography, "</strong><br>",
...
# asian_pm_mp <- girafe(ggobj = combined, width_svg = 12, height_svg = 5.5) %>%
#   girafe_options(opts_zoom(min = 1, max = 8),
#                  opts_selection(
#                    css = "fill:moccasin;",
#                    only_shiny = FALSE),
...
#                    opacity = 0.8,
#                    css = "background-color:#4c6061; color:white; padding:10px; border-radius:5px;"),
#                  opts_toolbar(pngname = "asian_pm_corr"))

# # ---- hispanic/latino
# hl_plot <- ggplot(data = community_districts, 
#                   aes(x = pct_hisp_latino/100, y = annual_mean_mcg_m3, color = pct_hisp_latino,
#                       data_id = communitydist)) +
#   stat_cor(method = "spearman") + 
#   geom_point_interactive(size = community_districts$total_pop/40000,
#                          aes(tooltip = paste0("<strong>Community District: ", geography, "</strong><br>",
...
# hisp_pm_mp <- girafe(ggobj = combined, width_svg = 12.5, height_svg = 5) %>%
#   girafe_options(opts_zoom(min = 1, max = 8),
#                  opts_selection(
#                    css = "fill:moccasin;",
#                    only_shiny = FALSE),
...
#                    opacity = 0.8,
#                    css = "background-color:#4c6061; color:white; padding:10px; border-radius:5px;"),
#                  opts_toolbar(pngname = "hisp_pm_corr"))

# # ---- black alone
# b_plot <- ggplot(data = community_districts, 
#                  aes(x = pct_black_alone/100, y = annual_mean_mcg_m3, color = pct_black_alone,
#                      data_id = communitydist)) +
#   stat_cor(method = "spearman") + 
#   geom_point_interactive(size = community_districts$total_pop/40000,
#                          aes(tooltip = paste0("<strong>Community District: ", geography, "</strong><br>",
...
# black_pm_mp <- girafe(ggobj = combined, width_svg = 12, height_svg = 5.5) %>%
#   girafe_options(opts_zoom(min = 1, max = 8),
#                  opts_selection(
#                    css = "fill:moccasin;",
#                    only_shiny = FALSE),
...
#                    opacity = 0.8,
#                    css = "background-color:#4c6061; color:white; padding:10px; border-radius:5px;"),
#                  opts_toolbar(pngname = "black_pm_corr"))

# ---- foreign born
fb_plot <- ggplot(data = community_districts,
                 aes(x = pct_foreign_born/100, y = annual_mean_mcg_m3, color = pct_foreign_born,
                     data_id = communitydist)) +
  stat_cor(method = "spearman") +
  geom_point_interactive(size = community_districts$total_pop/40000,
                         aes(tooltip = paste0("<strong>Community District: ", geography, "</strong><br>",
...
foreign_pm_mp <- girafe(ggobj = combined, width_svg = 12, height_svg = 5.5) %>%
  girafe_options(opts_zoom(min = 1, max = 8),
                 opts_selection(
                   css = "fill:moccasin;",
                   only_shiny = FALSE),
                 opts_tooltip(
                   opacity = 0.8,
                   css = "background-color:#4c6061; color:white; padding:10px; border-radius:5px;"),
                 opts_toolbar(pngname = "foreign_pm_corr"))

# # ---- SNAP households
# s_plot <- ggplot(data = community_districts, 
#                  aes(x = pct_households_snap/100, y = annual_mean_mcg_m3, color = pct_households_snap,
#                      data_id = communitydist)) +
#   stat_cor(method = "spearman") + 
#   geom_point_interactive(size = community_districts$total_pop/40000,
#                          aes(tooltip = paste0("<strong>Community District: ", geography, "</strong><br>",
...
# snap_pm_mp <- girafe(ggobj = combined, width_svg = 12, height_svg = 5.5) %>%
#   girafe_options(opts_zoom(min = 1, max = 8),
#                  opts_selection(
#                    css = "fill:moccasin;",
#                    only_shiny = FALSE),
...
#                    opacity = 0.8,
#                    css = "background-color:#4c6061; color:white; padding:10px; border-radius:5px;"),
#                  opts_toolbar(pngname = "snap_pm_corr"))
